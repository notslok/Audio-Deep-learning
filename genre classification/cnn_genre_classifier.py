...
def build_model(input_shape):
    ...
    # 1st conv layer
    model.add(keras.layers.Conv2D(32, (3, 3), activation='relu', input_shape=input_shape))    # -->  (# of kernel, grid size of each kernel, activation fn., input_shape)
    ...
    # 3rd conv layer
    model.add(keras.layers.Conv2D(32, (2, 2), activation='relu', input_shape=input_shape))  # -->  (# of kernel, grid size of each kernel, activation fn., input_shape)
    model.add(keras.layers.MaxPool2D((2, 2), strides=(2, 2), padding='same'))
    model.add(keras.layers.BatchNormalization())

    # flatten the output and feed it into dense layer
    model.add(keras.layers.Flatten())       # flattening the results of last conv layer
    model.add(keras.layers.Dense(64, activation='relu'))    # passing the flattened result through dense layer
...
if __name__ == "__main__":
    ...
    #  train the CNN
    model.fit(X_train, y_train, validation_data=(X_validation, y_validation), batch_size=32, epochs=30, verbose=1)   # verbose=1 will show the animated progress bar according o model.fit() documentation
